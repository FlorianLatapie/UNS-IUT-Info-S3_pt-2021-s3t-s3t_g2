...
class PaquetOutilsTest {
	...
	@Test
	void strToListEnum() {
		List<Statut> expected = new ArrayList<>();
		expected.add(Statut.ANNULEE);
		expected.add(Statut.COMPLETE);
		expected.add(Statut.TERMINEE);
		expected.add(Statut.ATTENTE);
		String message = "ANNULEE,COMPLETE,TERMINEE,ATTENTE";
		assertEquals(expected, PaquetOutils.strToListEnum(message, Statut.class));
		expected = new ArrayList<>();
		assertNotEquals(expected, PaquetOutils.strToListEnum(message, Statut.class));

		List<Statut> expected1 = new ArrayList<>();
		message = " ";
		assertEquals(expected1, PaquetOutils.strToListEnum(message, Statut.class));

		String message1 = "Annule,az";
		assertThrows(IllegalArgumentException.class, () -> PaquetOutils.strToListEnum(message1, Statut.class));
	}
	...
	@Test
	void listEnumToStr() {
		List<Statut> actual = new ArrayList<>();
		actual.add(Statut.ANNULEE);
		actual.add(Statut.COMPLETE);
		actual.add(Statut.TERMINEE);
		actual.add(Statut.ATTENTE);
		String expected = "ANNULEE,COMPLETE,TERMINEE,ATTENTE";
		assertEquals(expected, PaquetOutils.listEnumToStr(actual));
		expected = "";
		assertNotEquals(expected, PaquetOutils.listEnumToStr(actual));

		List<Statut> actual1 = new ArrayList<>();
		String expected1 = " ";
