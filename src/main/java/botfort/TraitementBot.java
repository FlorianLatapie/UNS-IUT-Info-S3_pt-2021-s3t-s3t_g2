...
public class TraitementBot {
	...
	public void initialiserPartie(BotFort core, List<?> nomsT, List<Couleur> couleursT, int lieuferme) {
	...
	public void lancerDes(BotFort core, List<?> pionT) {
	...
	public int choisirPionPlacement(BotFort core) {
		int pion = 0;
	...
	public void debutTour(BotFort core, List<Couleur> couleurs) {
	...
	public int choixDest(BotFort core) {
	...
	public int choixBestDest (BotFort core) {
		out.println("Entrez une destination");
		int dest = 0;
		int lieuMin =20;
	...
	public void debutDeplacemant(BotFort core, List<?> lieuxT) {
	...
	public List<Object> pionADeplacer(BotFort core, int dest, HashMap<Integer, List<Integer>> listedp) {
	...
	public void attaqueZombie(BotFort core, List<PionCouleur> l, List<PionCouleur> ltemp) {
	...
	public PionCouleur choisirSacrifice(BotFort core, List<?> listPionT) {
		if (listPionT.size()>0) {
	...
	public void finPartie(BotFort core, Couleur gagnant) {
		out.println("Le gagant est le joueur " + gagnant + " !");
		core.setEstFini(true);
		// getControleurReseau().arreter();
	}

	public List<Object> listeCarteJouee(BotFort core, int n) {
		List<Object> listRenvoye = new ArrayList<>();
	...
	public List<PionCouleur> listePionCache(BotFort core) {
	...
	public CarteType ReponseJoueurCourant(BotFort core) {
	...
	public int IndiquerCarteJouees(BotFort core) {
	...
	public Couleur getRandom(BotFort core, VoteType vt) {
		int rand;
		if (vt == VoteType.MPZ) {
	...
	public Couleur getRandom2(BotFort core, VoteType vt) {
		int rand = new Random().nextInt(core.getJoueurEnVie().size());
		return core.getJoueurEnVie().get(rand);

	}

	public List<Object> carteFouille(List<CarteType> listeCarte, BotFort bot) {
