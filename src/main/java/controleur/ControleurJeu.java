...
public class ControleurJeu {
	...
	public void demarerJeu() {
		ThreadTool.asyncTask(() -> {
			...
			String m = nwm.getPacketsTcp().get("IP").build(getJoueursListeNo(), getCouleurJoueursListeNo(), 0, 3,
					partieId);
			for (Joueur j : jeu.getJoueurs().values())
		...
		for (int i = 0; i < jeu.getJoueurs().size(); i++) {
			if (jeu.getJoueurs().get(i).isChefDesVigiles() && jeu.getJoueurs().get(i).isEnVie()) {
				...
				m = nwm.getPacketsTcp().get("DPD").build(destination.get(compteur),
						jeu.allChoixPossible(jeu.getJoueurs().get(i)), partieId, numeroTour);
				String message = TcpClientSocket.connect(jeu.getJoueurs().get(i).getIp(),
						jeu.getJoueurs().get(i).getPort(), m, null, 0);
	...
	private void placementPersonnage() {
		for (int n = 0; n < jeu.getJoueurs().get(0).getPersonnages().size(); n++) {
			for (int i = 0; i < jeu.getJoueurs().size(); i++) {
				...
				for (Joueur j : jeu.getJoueurs().values())
					if (j != jeu.getJoueurs().get(i))
						ThreadTool.taskPacketTcp(j.getIp(), j.getPort(), message);
