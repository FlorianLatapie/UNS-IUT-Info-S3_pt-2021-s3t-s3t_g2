...
public class ControleurJeu {
    ...
    public ControleurJeu() throws IOException {
        ...
        ThreadTool.asyncTask(() -> {
            ...
            String m = nwm.getPacketsUdp().get("ACP").build(
                    intPartieId, nwm.getAddress().getHostAddress(), port, nomPartie, nbjtotal, nbjr, nbjv, status);
    ...
    private ArrayList<Joueur> createJoeur() {
        ...
        for (int i = 0; i < nbjr; i++) {
            ...
            //listeJoueur.add(new Joueur("a", "12", -1, listeCouleur.get(choix), "joueur " + i));
