        ...
        for (int i = 0; i < jeu.getJoueurs().size(); i++) {
            ...
            if (jeu.getJoueurs().get(i).isEnVie()) {
                ...
                if (!jeu.getLieux().get(destination.get(compteur)).isFull()
                    ...
                    out.println(MessageFormat.format("{0} choisit un personage a déplacer à {1}", jeu.getJoueurs().get(i), jeu.getLieux().get(destination.get(compteur))));
                } else {
                ...
                while (!pers.contains(choixPerso)) {
                    ...
                    for (Integer per : pers) {
                        out.println(
                                per + "\t" + jeu.getJoueurs().get(i).getPersonnages().get(per));
                    }
                    // choixPerso = sc.nextInt();
                    choixPerso = new Random().nextInt(3); // temporaire
                    out.println(choixPerso); // temporaire
                ...
                if (i == 4) {// si parking
                    for (int j = 0; j < jeu.getLieux().get(i).getNbZombies(); j++) {
                        if (!jeu.getLieux().get(i).getPersonnage().isEmpty()) {
                            ...
                            out
                                    .println(MessageFormat.format("{0} choisis un numéro a sacrifier a {1}: ", jou, jeu.getLieux().get(i)));
                            ...
                            while (!num.contains(persEntre)) {
                                ...
                                out.println("Numéro incorect !\n");
                                out.println(
                                        MessageFormat.format("{0} choisis un numéro a sacrifier a {1}: ", jou, jeu.getLieux().get(i)));
